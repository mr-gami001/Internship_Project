...
class _loginState extends State<login> {
  ...
  Widget elevatedchild = Text('Login',style: TextStyle(fontSize: 20),);
          ...
          Container(
            ...
            child: ElevatedButton(
              ...
              onPressed: () async {
                ...
                setState(() {
                  elevatedchild =CircularProgressIndicator(color: Colors.white,);
                });
                if(dropdownValue == 'Institute'){
                  String Credential = await institutelog(name.text, password.text);
                  // if(Credential == 'okay'){
                ...
                if(Credential == 'okay'){
                  ...
                  print("shared preference data : *** ${logindata.getString('user')} *** ${logindata.getString('userid')}");
                  navigator(dropdownValue,name.text,context);
                }
                  else{
                    showDialog(context: context, builder: (context)=>AlertDialog(title: Text(Credential),));
                    setState(() {
                      elevatedchild =Text('Login',style: TextStyle(fontSize: 20));
                    });
                  }
                }else {


                  String check = await loginservice(dropdownValue.toLowerCase(), name.text, password.text);
                  if(check == 'okay'){
                    Map<String,String> userdata = {
                      'user' : dropdownValue.toString(),
                      'userid' : name.text.toString()
                    ...
                    await showDialog(context: context, builder: (context)=>AlertDialog(title: Text(check),));
                    setState(() {
                      elevatedchild = Text('Login');
                    });
                  }
